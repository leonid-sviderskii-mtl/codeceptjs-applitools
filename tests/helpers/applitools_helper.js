...
class ApplitoolsHelper extends Helper {
    ...
    /**
    * @param testName {String} name of the page you want to check
    ...
    * 
     */
    async eyeCheck({ testName, element, uniqueId, matchLevel }) {
        eyes.setApiKey(this.config.applitoolsKey);

        if (uniqueId) {
            eyes.setBatch(testName, uniqueId); 
        }
        ...
        await eyes.open(client, appName, testName, windowsSize);


        if (element) {
            eyes.setForceFullPageScreenshot(false);
            await eyes.check(testName, Target.region(element));
        } else {
            eyes.setForceFullPageScreenshot(true);
            await eyes.check(testName, Target.window());
        }
